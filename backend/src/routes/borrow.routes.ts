...
export function borrowRoutes(method: string, path: string, query: URLSearchParams, user: never, body: Record<string, unknown>) {
  ...
  if (method === "PATCH" && path.startsWith("/api/borrow/") && path.endsWith("/approve")) return borrowController.approve(user, path.split("/").at(-2) ?? "", body.approved !== false);
  if (method === "PATCH" && path.startsWith("/api/borrow/") && path.endsWith("/return")) return borrowController.confirmReturn(user, path.split("/").at(-2) ?? "", String(body.condition ?? "Good"));
  return undefined;
}
