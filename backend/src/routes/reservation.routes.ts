...
export function reservationRoutes(method: string, path: string, query: URLSearchParams, user: never, body: Record<string, unknown>) {
  ...
  if (method === "PATCH" && path.startsWith("/api/reservations/") && path.endsWith("/approve")) return reservationController.approve(user, path.split("/").at(-2) ?? "", body.approved !== false);
  if (method === "PATCH" && path.startsWith("/api/reservations/") && path.endsWith("/cancel")) return reservationController.cancel(user, path.split("/").at(-2) ?? "");
  return undefined;
}
